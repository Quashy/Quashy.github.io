...
| 编号 | 用例名称 | 输入示例 | 预期结果 |
| --- | --- | --- | --- |
| TC01 | 正常顺序计算 | 初始 `inside=0`；`10:00 enter=5 exit=0`；`10:05 enter=0 exit=3` | `inside=2` |
| TC02 | 离开人数超过当前人数 | 初始 `inside=3`；`10:00 enter=0 exit=5` | `inside=0`，不能为负数 |
| TC03 | 单条消息同时有进入和离开 | 初始 `inside=3`；`10:00 enter=2 exit=5` | 先加进入再扣离开，`inside=0` |
| TC04 | 基础乱序：离开先到，进入后到 | MQ 到达：`10:05 exit=5`，`10:00 enter=5` | 按事件时间重放后 `inside=0` |
| TC05 | 迟到进入影响后续多条事件 | MQ 到达：`10:05 exit=3`，`10:10 exit=2`，`10:00 enter=5` | 重放顺序为 `10:00 -> 10:05 -> 10:10`，最终 `inside=0` |
| TC06 | 迟到离开修正当前人数 | MQ 到达：`10:00 enter=10`，`10:10 enter=5`，`10:05 exit=8` | 重放后 `inside=7` |
| TC07 | 多条迟到事件连续到达 | MQ 到达：`10:15 enter=4`，`10:05 exit=2`，`10:00 enter=3`，`10:10 enter=1` | 按事件时间重放后 `inside=6` |
| TC08 | 重复消息幂等 | 同一个 `eventId=e1` 的 `10:00 enter=5` 消费两次 | 只计算一次，`inside=5` |
| TC09 | 相同 `eventId` 但内容冲突 | 第一次：`eventId=e1 enter=5`；第二次：`eventId=e1 enter=7` | 不允许静默覆盖；应拒绝、告警或进入异常记录 |
| TC10 | 同一事件时间多条消息 | 初始 `inside=0`；`10:00 event-a exit=5`；`10:00 event-b enter=5` | 按 `happenTime + eventId` 稳定排序，多次重放结果一致 |
| TC11 | 迟到事件早于最近快照 | 已有快照：`10:10 inside=3`；新到迟到事件：`10:05 enter=5` | 不能直接使用 `10:10` 快照；需要使后续快照失效或从更早快照重放 |
| TC12 | 新区域首次上报 | `areaCode=area-new`；`10:00 enter=4 exit=0`；当前无状态行 | 创建区域状态，`inside=4`，`lastCalculatedTime=10:00` |
| TC13 | 不同区域隔离计算 | `area-a 10:00 enter=5`；`area-b 10:00 exit=3` | `area-a inside=5`；`area-b inside=0`，互不影响 |
| TC14 | 同一区域并发消费 | 初始 `inside=0`；线程 A 处理 `10:00 enter=5`；线程 B 处理 `10:05 enter=3` | 同一区域串行化，最终 `inside=8`，不能出现覆盖写 |
| TC15 | 原始事件入库成功但状态更新失败 | `10:00 enter=5` 入库成功，更新状态失败后重试 | 可基于事件表恢复，最终只计算一次 |
| TC16 | MQ ACK 失败导致重投 | 第一次状态已更新但 ACK 失败；MQ 再次投递同一 `eventId` | 第二次幂等跳过，不重复增加人数 |
| TC17 | 缺失事件时间 | 消息无 `happenTime` | 拒绝处理，不能降级使用 MQ 到达时间 |
| TC18 | 非法人数 | `enter=-1` 或 `exit=-2` | 拒绝处理，进入异常记录 |
| TC19 | 超大人数防溢出 | `enter=Long.MAX_VALUE` 或接近上限 | 不发生数值溢出；应拒绝或按明确策略处理 |
| TC20 | 重放确定性 | 同一批历史事件重复回放多次 | 每次得到相同 `inside` 和相同 `lastCalculatedTime` |
